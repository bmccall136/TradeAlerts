...
# Replace the entire realized_buckets_from_live_db(...) function with a DB-only implementation.
# This prevents recursion and matches your desired method: compute buckets from live.db realized_trades.

pat = re.compile(
    r'(?ms)^def\s+realized_buckets_from_live_db\s*\(.*?\)\s*:\s*\n'
    r'(?:^[ \t].*\n)+'
)

replacement = r'''def realized_buckets_from_live_db(db_path, now=None):
    """
...
print("OK replaced realized_buckets_from_live_db() with DB-only implementation")
