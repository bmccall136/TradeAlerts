...
def main():
    ap = argparse.ArgumentParser(description="List open (working) orders; default filters to MRNA.")
    ap.add_argument("--symbol", default="MRNA", help="Filter by symbol (default: MRNA). Use ALL to show everything.")
    args = ap.parse_args()
    ...
    list_funcs = [getattr(et, n) for n in [
        "list_orders", "get_orders", "list_open_orders", "get_order_list", "orders"
    ] if hasattr(et, n)]
    ...
    for fn in list_funcs:
        try:
            ...
            kwargs = {}
            if "account_id_key" in params: kwargs["account_id_key"] = aid
    ...
    def iter_orders(payload):
        if isinstance(payload, dict):
            for k in ("OrdersResponse","OrderListResponse","orderList","Orders","orders"):
                if k in payload:
                    payload = payload[k]
                    break
    ...
    # Extract key fields
    def info(row):
        oid = row.get("orderId") or row.get("order_id") or row.get("OrderId") or row.get("id")
        status = row.get("status") or row.get("orderStatus") or row.get("order_status")
        sym = (row.get("symbol") or (row.get("Product") or {}).get("symbol") or
               (row.get("Instrument") or {}).get("symbol"))
        side = row.get("side") or row.get("action") or row.get("orderAction")
        qty  = row.get("qty") or row.get("quantity") or row.get("orderedQuantity") or row.get("filledQuantity")
        ...
        ts    = row.get("placedTime") or row.get("timePlaced") or row.get("transactionDate")
        return {
            "orderId": oid, "status": status, "symbol": sym, "side": side,
            "qty": qty, "priceType": ptype, "price": price, "stopPrice": stop, "placed": ts
        }
    ...
    live_keywords = {"OPEN","WORKING","PENDING","QUEUED","ACCEPTED","LIVE","PARTIAL"}
    live = [r for r in rows if (str(r["status"] or "").upper() in live_keywords or
                                any(k in str(r["status"] or "").upper() for k in live_keywords))]
    ...
    print("Or run cancel_open_orders.py --symbol", args.symbol, "to cancel all open orders for that symbol.")
