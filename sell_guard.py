...
# sell_guard.py  — LIVE sell guard with immediate trailing arm on gain (Ben 2025-11-04)
...
DEFAULTS = {
  ...
  "sell_guard": {
    ...
    "min_hold_minutes": 0,
    "normalize_tick": 0.01,
    "sell_protect_after_mins": 0,   # ignored for arming trail
    "sell_window_start_et": "04:00",
    "sell_window_end_et": "20:00",
    "stop_bps": 100,
    "target_bps": 200,
...
def iter_positions(pobj):
    """Yield (sym, qty_float, last, entry) from E*TRADE shapes."""
...
TRAIL: Dict[str, Dict[str, float]] = {}  # sym -> {"hi": high_pl_pct, "trail_pct": float, "armed": 0/1}
...
def main():
    ...
    cfg = GuardSettings(
        sell_window_start_et = sg.get("sell_window_start_et") or cfg_raw.get("sell_window_start_et") or "04:00",
        sell_window_end_et   = sg.get("sell_window_end_et")   or cfg_raw.get("sell_window_end_et")   or "20:00",
    ...
    while True:
        ...
        # opened_at map from guardrails table
        ...
        for sym, q, last, entry in rows:
            ...
            if not fwe and sym not in open_map:  # must have an opened_at unless forced
                continue
            if q < 1: continue
            pl_pct = ((last - entry) / max(entry, 0.0001) * 100.0) if (last > 0 and entry > 0) else 0.0
            hold_min = -1.0
            if sym in open_map:
                try:
                    ...
                    hold_min = -1.0
            candidates.append((sym, int(q), pl_pct, hold_min, entry))
        ...
        # New: immediate arm thresholds
        arm_at          = float(cfg.sg.get("protect_trail_arm_pct", 3.0))    # e.g., 3.0 (%)
        trail_pct_cfg   = float(cfg.sg.get("protect_trail_pct", 3.0))        # e.g., 3.0 (%)
        min_gain_lock   = float(cfg.sg.get("protect_min_gain_pct", 1.5))     # log only
        ...
        for s, qty, pl_pct, hold_min, entry_px in candidates:
            ...
            # Protective STOP regardless of hold (intraday stoploss)
            if allow_intraday_stop and pl_pct <= -stop_pct:
                pt, lim = compute_order_params(s, cfg)
                LOG.info("[STOP] %s %.2f%% ≤ -%.2f%% → place %s%s", s, pl_pct, stop_pct, pt, f" {lim:.2f}" if lim else "")
            ...
            # --- Trailing logic ---
            # Always record high-water gain
            cur = pl_pct
            hi = TRAIL.get(s, {}).get("hi", cur)
            hi = max(hi, cur)
            TRAIL.setdefault(s, {})["hi"] = hi

            # Arm immediately when gain >= arm threshold (ignores min_hold)
            if cur >= arm_at and not TRAIL[s].get("armed"):
                TRAIL[s]["armed"] = 1
                TRAIL[s]["trail_pct"] = max(0.5, trail_pct_cfg)
                LOG.info("[TRAIL_ARM] %s gain=%.2f%% ≥ %.2f%% → armed trail=%.2f%% (hi=%.2f%%)",
                         s, cur, arm_at, TRAIL[s]['trail_pct'], hi)

            # If armed, sell when drawdown from HI ≥ trail_pct
            if TRAIL[s].get("armed"):
                trail = TRAIL[s]["trail_pct"]
                if cur <= (hi - trail):
                    pt, lim = compute_order_params(s, cfg)
                    LOG.info("[TRAIL_SELL] %s gain %.2f%% fell from hi %.2f%% by ≥ %.2f%% → place %s%s",
                             s, cur, hi, trail, pt, f" {lim:.2f}" if lim else "")
                    place_with_adaptive_variants(acct_key, s, qty, pt, (lim if pt=='LIMIT' else None), cfg.max_place_attempts)
                    # keep armed state; hi will reset on next loop if still a position
                    continue
                else:
                    LOG.info("[ARMED] %s hi=%.2f%% gain=%.2f%% trail=%.2f%% (holding)", s, hi, cur, trail)
                    continue

            # Not armed yet → respect min_hold before acting on targets (but only for sells other than trail)
            if hold_min >= 0 and hold_min < min_hold:
                LOG.info("[HOLD] %s hold %.1f mins < %.1f mins (min_hold); gain=%.2f%%", s, hold_min, min_hold, cur)
                continue

            # Optional fixed target exit (if you want it alongside trailing)
            if cur >= target_pct:
                pt, lim = compute_order_params(s, cfg)
                LOG.info("[TARGET] %s gain %.2f%% ≥ %.2f%% → place %s%s", s, cur, target_pct, pt, f" {lim:.2f}" if lim else "")
                place_with_adaptive_variants(acct_key, s, qty, pt, (lim if pt=='LIMIT' else None), cfg.max_place_attempts)
                continue

            # Otherwise do nothing
            LOG.info("[HOLD] %s gain=%.2f%% hold=%.1f mins (waiting for trail/target/timeout)", s, cur, hold_min)
