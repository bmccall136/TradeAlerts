...
bak = P.with_suffix(f".html.bak_trv_trade_api_render_v1_{ts}")
shutil.copy2(P, bak)
print("Backup ->", bak)

txt = P.read_text(encoding="utf-8", errors="replace")

MARK = "MM_TRV_TRADE_RENDER_FROM_API_V1"
if MARK in txt:
...
(function(){
  ...
  // trade_id is in the URL path: /trade_review/trade/<id>
  ...
  fetch("/api/trade_review?trade_id=" + encodeURIComponent(trade_id))
    ...
    .then(function(j){
      // Find the headings (works even if structure changes a bit)
      ...
      // Update the "Buy: @ 0.00" / "Sell: @ 0.00" line if present
      var meta = j.meta || {};
