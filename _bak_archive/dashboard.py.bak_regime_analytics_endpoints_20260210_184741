...
def trade_review_chart():
    ...
    try:
        ...
        # Optional "ts" marker (yellow-ish label); does NOT break anything if absent
        if ts:
            t = _to_index_tz(ts)
            if t:
...
def api_analytics_candidate_fires():
    """
    Counts indicator fires from triggers_*.csv (candidate rows).
    Bucket: today / 7d / 30d / all  (defaults to 7d)
    """
    ...
    def _bucket_days(b: str):
        if b in ("all",):
            return None
        if b in ("today",):
            return 0
        if b.endswith("d") and b[:-1].isdigit():
            return int(b[:-1])
        return 7

    days = _bucket_days(bucket)
    now = datetime.now(timezone.utc)

    def _file_ok(fp: str) -> bool:
        if days is None:
            return True
        base = os.path.basename(fp)
        m = re.search(r"triggers_(\d{4}-\d{2}-\d{2})\.csv", base)
        if not m:
            return False
        try:
    ...
    # Normalize from signals_pretty tokens into families
    def _norm_token(t: str) -> str | None:
        tl = (t or "").strip().lower()
        if not tl:
            return None
        tl = tl.replace(" ", "")
        if "price>sma" in tl or tl.startswith("sma") or "sma(" in tl:
            return "sma"
        if "vwap" in tl:
            return "vwap"
        if "macd" in tl:
        ...
        if "rsi" in tl:
            return "rsi"
        if "vol" in tl or "volume" in tl:
            return "vol"
        if "bb" in tl or "boll" in tl:
            return "bb"
        if "adx" in tl:
            return "adx"
        if "gap" in tl:
    ...
    files = sorted(glob.glob(os.path.join(logs_dir, "triggers_*.csv")))
    for fp in files:
        if not _file_ok(fp):
            continue
        try:
            with open(fp, "r", encoding="utf-8", newline="") as f:
                ...
                sigk = next((k for k in cols if "signals_pretty" in (k or "").lower() or (k or "").lower() == "signals"), None)
                for row in r:
                    total_rows += 1
                    raw = (row.get(sigk) if sigk else "") or ""
                    toks = _split_signals(raw)
                    families = set()
                    for t in toks:
                        fam = _norm_token(t)
                        if fam:
    ...
    items = [{"indicator": k, "n": int(v)} for k, v in sorted(counts.items(), key=lambda kv: (-kv[1], kv[0]))]
    return jsonify({"ok": True, "bucket": bucket, "total": int(total_rows), "items": items})

@app.route("/api/analytics/buy_funnel")
def api_analytics_buy_funnel():
    """
    Candidate -> funnel summary (bucketed).
    Candidates come from triggers_*.csv rows (post-required gate).
    Strict-pass approximated by signals_pretty token count >= strict_buy_signals.
    Bought is best-effort from live.db (optional; returns 0 if not found).
    """
    import os, glob, csv, re, sqlite3
    from datetime import datetime, timedelta, timezone
    from flask import jsonify, request
        ...
        try:
            with open(fp, "r", encoding="utf-8", newline="") as f:
                ...
                for row in r:
                    candidates += 1
                    raw = (row.get(sigk) if sigk else "") or ""
                    n = len(_split_signals(raw))
                    if n >= strict_n:
                        strict_pass += 1
        except Exception:
            continue

    # Best-effort bought count from live.db (safe = never raises)
    bought = 0
    try:
        if os.path.exists(live_db):
            ...
            t = None
            for cand in ("trades", "executed_trades", "order_fills", "fills", "orders"):
                if cand in tables:
                    t = cand
                    break
            ...
            if t:
                ...
                side_col = next((c for c in ("action","side","type") if c in cols), None)
                ts_col   = next((c for c in ("ts_utc","time_utc","created_utc","timestamp","time","time_et","ts_et") if c in cols), None)

                wh = []
                params = []

                if side_col:
                    wh.append(f"LOWER({side_col}) IN ('buy','b')")

                if ts_col and days is not None:
                    cutoff = (now - timedelta(days=days)).isoformat()
                    wh.append(f"julianday({ts_col}) >= julianday(?)")
                    params.append(cutoff)

                where_sql = (" WHERE " + " AND ".join(wh)) if wh else ""
                cur.execute(f"SELECT COUNT(*) FROM {t}{where_sql}", params)
                bought = int(cur.fetchone()[0] or 0)

            con.close()
    except Exception:
        bought = 0

    # derived stages (keep simple + non-noisy)
    required_pass = candidates         # triggers_*.csv implies passed required gate
    buyable = strict_pass              # conservative approximation
...
# -------------------------
# Daily Analytics API (schema-adaptive)
# BUYS: position_opened (opened timestamp)
# SELLS: realized_trades (close timestamp)
# -------------------------
...
def api_analytics_daily():
    ...
    date_arg = (request.args.get("date") or "").strip()  # YYYY-MM-DD
    if not date_arg:
    ...
    if not db_path:
        return jsonify({"error": "No LIVE_DB/DB_PATH configured", "date": date_arg}), 500
    ...
    try:
        # ---------- BUYS ----------
# ---------- BUYS ----------
        # Primary source: position_opened
        po_cols = table_cols(cur, "position_opened")
        po_ts   = pick(po_cols, "opened_utc", "opened_at", "ts_utc", "ts", "time_utc")
        po_sym  = pick(po_cols, "symbol", "ticker")
        po_name = pick(po_cols, "name", "company_name")
        po_qty  = pick(po_cols, "qty", "quantity", "shares")
        po_px   = pick(po_cols, "price", "entry_price", "fill_price", "avg_price")
        ...
        if po_ts and po_sym:
            sel = [
                f"{po_ts} AS ts",
                f"{po_sym} AS symbol",
                (f"{po_name} AS name" if po_name else "'' AS name"),
                (f"{po_qty} AS qty"   if po_qty  else "NULL AS qty"),
                (f"{po_px} AS price"  if po_px   else "NULL AS price"),
                "'position_opened' AS src"
            ]
            sql = "SELECT " + ", ".join(sel) + f" FROM position_opened WHERE date({po_ts})=? ORDER BY {po_ts} ASC"
            cur.execute(sql, (date_arg,))
            buys_po = [dict(r) for r in cur.fetchall()]

        # Secondary source (manual reimport days): trades action='BUY'
        tr_cols = table_cols(cur, "trades")
        tr_ts   = pick(tr_cols, "ts_utc", "ts_et", "ts", "time_utc", "time_et", "created_at")
        tr_sym  = pick(tr_cols, "symbol", "ticker")
        tr_name = pick(tr_cols, "name", "company_name")
        tr_qty  = pick(tr_cols, "qty", "quantity", "shares")
        tr_px   = pick(tr_cols, "price", "fill_price", "avg_price", "entry_price")
        tr_act  = pick(tr_cols, "action", "side", "type", "transactionType", "transaction_type")

        buys_tr = []
        if tr_ts and tr_sym and tr_act:
            sel = [
                f"{tr_ts} AS ts",
                f"{tr_sym} AS symbol",
                (f"{tr_name} AS name" if tr_name else "'' AS name"),
                (f"{tr_qty} AS qty"   if tr_qty  else "NULL AS qty"),
                (f"{tr_px} AS price"  if tr_px   else "NULL AS price"),
                "'trades' AS src"
            ]
            sql = (
                "SELECT " + ", ".join(sel) +
                f" FROM trades WHERE date({tr_ts})=? AND {tr_act} IN ('BUY','Buy','B','Bought','Open') ORDER BY {tr_ts} ASC"
            )
            cur.execute(sql, (date_arg,))
            buys_tr = [dict(r) for r in cur.fetchall()]

        # Merge + de-dupe (prefer position_opened for same (symbol, ts))
        merged = []
        seen = set()  # (symbol, ts)
        for r in buys_po:
            k = (str(r.get('symbol','')), str(r.get('ts','')))
        ...
        # ---------- SELLS ----------
        rt_cols = table_cols(cur, "realized_trades")
        rt_ts   = pick(rt_cols, "close_date", "closed_utc", "ts_utc", "ts", "time_utc")
        rt_sym  = pick(rt_cols, "symbol", "ticker")
        rt_name = pick(rt_cols, "name", "company_name")
        rt_qty  = pick(rt_cols, "qty", "quantity", "shares")
        rt_open = pick(rt_cols, "open_price", "entry_price", "buy_price", "avg_open", "cost_basis")
        rt_close= pick(rt_cols, "close_price", "exit_price", "sell_price", "avg_close")
        rt_gain = pick(rt_cols, "gain", "pnl", "pl", "profit")
        rt_gpct = pick(rt_cols, "gain_pct", "pnl_pct", "pl_pct", "profit_pct")

        if not rt_ts or not rt_sym:
            sells = []
        else:
            sel = [
                f"{rt_ts} AS ts",
            ...
            if rt_name: sel.append(f"{rt_name} AS name")
            else:       sel.append("'' AS name")
            if rt_qty:  sel.append(f"{rt_qty} AS qty")
            else:       sel.append("NULL AS qty")
            if rt_open: sel.append(f"{rt_open} AS open_price")
            else:       sel.append("NULL AS open_price")
            if rt_close:sel.append(f"{rt_close} AS close_price")
            else:       sel.append("NULL AS close_price")
            if rt_gain: sel.append(f"{rt_gain} AS gain")
            else:       sel.append("NULL AS gain")
            if rt_gpct: sel.append(f"{rt_gpct} AS gain_pct")
            else:       sel.append("NULL AS gain_pct")
        ...
        return jsonify({
            ...
            "meta": {
                "db": str(db_path),
                "position_opened_ts": po_ts,
                "realized_trades_ts": rt_ts
            }
        })
...
# --- SELL_ANALYTICS_API_V1 (LIVE_DB sell_events) ---
@app.get("/api/analytics/sell")
def api_analytics_sell():
    """
    Sell Analytics (LIVE_DB-only)
    - exit_reason_breakdown: sell_events where event='FILLED'
    - execution_outcomes: sell_events all events
    - hold_buckets: realized_trades open_date/close_date (best-effort)
    """
    try:
        import sqlite3
        from datetime import datetime, timedelta, timezone
        ...
        bucket = (request.args.get("bucket") or "7d").lower().strip()
        # Support bucket=today (midnight ET -> now, converted to UTC epoch)
        start_epoch_today = None
        try:
            from zoneinfo import ZoneInfo
            etz = ZoneInfo("America/New_York")
            now_et = datetime.now(etz)
            mid_et = now_et.replace(hour=0, minute=0, second=0, microsecond=0)
            start_epoch_today = int(mid_et.astimezone(timezone.utc).timestamp())
        except Exception:
            start_epoch_today = None

        days_map = {"today": 0, "7d": 7, "30d": 30, "all": None}
        days = days_map.get(bucket, 7)

        now_utc = datetime.now(timezone.utc)
        start_epoch = start_epoch_today if bucket=="today" else (int((now_utc - timedelta(days=days)).timestamp()) if days else None)

        # HARD-PIN the DB path (no ambiguity)
        db_path = r"C:\TradeAlerts\live.db"
        ...
        try:
            ...
            if start_epoch is None:
                rows = con.execute("SELECT event FROM sell_events").fetchall()
            else:
                rows = con.execute("SELECT event FROM sell_events WHERE ts_utc >= ?", (start_epoch,)).fetchall()
            outcomes = Counter([r[0] for r in rows if r and r[0]])

            # ---- exit reasons (FILLED only) ----
            if start_epoch is None:
                rows2 = con.execute("SELECT reason FROM sell_events WHERE event='FILLED'").fetchall()
            else:
                rows2 = con.execute("SELECT reason FROM sell_events WHERE event='FILLED' AND ts_utc >= ?", (start_epoch,)).fetchall()
            reasons = Counter([(r[0] or "UNKNOWN") for r in rows2])

            # ---- hold-time buckets (realized_trades) ----
            def _parse_dt(s):
                if not s:
                    return None, "missing"
                s = str(s).strip()
                ...
                try:
                    ...
                    if d.tzinfo is None:
                        return d, "naive_ok"
                    return d.astimezone(timezone.utc).replace(tzinfo=None), "aware_ok"
                except Exception:
                    pass
                for f in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%d"):
                    try:
                        d = datetime.strptime(s, f)
                        return d, ("date_only" if f == "%Y-%m-%d" else "naive_ok")
                    except Exception:
                        continue
                return None, "bad"

            rt = con.execute("SELECT symbol, action, open_date, close_date FROM realized_trades").fetchall()
            ...
            for sym, act, od, cd in rt:
                if not cd:
                    continue
                if act and str(act).upper() in ("BUY","B"):
                    continue

                odt, of = _parse_dt(od)
                cdt, cf = _parse_dt(cd)
                if odt is None or cdt is None or of != "naive_ok" or cf != "naive_ok":
                    unknown += 1
                    continue

                try:
                    # interpret naive as UTC (consistent); used only for duration
                    mins = (cdt - odt).total_seconds() / 60.0
                    if mins < 0:
                        unknown += 1
                        continue
                except Exception:
            ...
            if unknown:
                hold_counts["UNKNOWN"] += int(unknown)

            order = ["<5m","5?15m","15?30m","30?60m","1?2h","2?4h",">4h","UNKNOWN"]
            hold_buckets = [{"bucket": k, "ct": int(v)} for k, v in hold_counts.items()]
            hold_buckets.sort(key=lambda x: order.index(x["bucket"]) if x["bucket"] in order else 999)

            dbg = {
                "sell_events_total": int(con.execute("SELECT COUNT(*) FROM sell_events").fetchone()[0]),
                "realized_trades_total": int(con.execute("SELECT COUNT(*) FROM realized_trades").fetchone()[0]),
            }
        ...
        return jsonify({
            "meta": {"bucket": bucket, "start_epoch": start_epoch, "db_path": db_path},
            "exit_reason_breakdown": dict(reasons),
            "execution_outcomes": dict(outcomes),
            "hold_buckets": hold_buckets,
            "debug": dbg,
            "v": "SELL_ANALYTICS_API_V1",
        })

    except Exception as e:
        return jsonify({"error": str(e), "v": "SELL_ANALYTICS_API_V1"}), 500

# --- /SELL_ANALYTICS_API_V1 ---

# ------------------------------------------------------------
# Chart view (required by /trade_review redirects)
# ------------------------------------------------------------

@app.route("/chart_img/<symbol>")
def chart_img_symbol(symbol):
    """
    Trade Review chart IMAGE endpoint (for <img src=...>).

    Delegates to the existing PNG chart endpoint (/chart?symbol=...),
    so we keep ONE chart renderer and avoid referrer/symbol ambiguity.
    """
    ...
    # Explicit symbol in querystring = no referrer hacks
    url = f"/chart?symbol={quote(sym)}"
    if ts:
        url += f"&ts={quote(ts)}"
    if date:
...
def chart_symbol(symbol):
    ...
    # Chart route is an alias for Trade Review (single source of truth for rendering)
    if ts and date:
        return redirect(f"/trade_review?symbol={symbol}&date={date}&ts={ts}", code=302)
    if ts:
        return redirect(f"/trade_review?symbol={symbol}&ts={ts}", code=302)
    if date:
        return redirect(f"/trade_review?symbol={symbol}&date={date}", code=302)
    return redirect(f"/trade_review?symbol={symbol}", code=302)


# ------------------------------------------------------------
# Dev server entrypoint (added to ensure dashboard starts)
# ------------------------------------------------------------
if __name__ == "__main__":
    # NOTE: keep reloader OFF on Windows to avoid duplicate listeners
