...
# - Fresh clientOrderId in PREVIEW and PLACE
# - Minimal PLACE body (no read-only totals)
# - Price fields as 2-decimal strings when present
...
def build_min_place_body(
    preview: dict, client_id: str, qty_override: int | None = None
) -> dict:
    ...
    new_instr = [
        {
            ...
            "quantity": str(
                int(
                    ...
                    else instr[0].get("quantity") or 1
                )
            ),
        }
    ]
...
def main():
    ...
    if avail < args.qty:
        print(f"[ABORT] Requested qty {args.qty} exceeds available_to_sell {avail}")
        return
    ...
    if args.market:
        ...
        bid = best_bid(SYMBOL)
        if bid <= 0:
            print("[ABORT] No valid bid (use --market or --limit)")
            return
