...
def daily_loss_cap_breached(trade_log, holdings, max_daily_loss=-1000):
    ...
    realized = sum(
        ...
        and datetime.fromisoformat(t["trade_time"]).date() == today
    )
    unrealized = sum((h["last_price"] - h["price_paid"]) * h["qty"] for h in holdings)
...
def wash_sale_prohibited(
    symbol: str, current_date: datetime, trade_log: list[dict], days: int = 30
) -> bool:
    """
...
def funds_not_settled(
    symbol: str, current_date: datetime, trade_log: list[dict], settlement_days: int = 2
) -> bool:
    """
    ...
    last_buy = next(
        (
            ...
            and t.get("trade_time") is not None
        ),
        None,
    )
    if not last_buy:
...
def enforce_wash_sale(
    symbol: str, trade_log: list[dict], buy_date: datetime, days: int = 30
) -> bool:
...
def enforce_settlement(
    symbol: str, trade_log: list[dict], current_time: datetime, settlement_days: int = 2
) -> bool:
