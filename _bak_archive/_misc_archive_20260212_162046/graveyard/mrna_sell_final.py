...
def main():
    ...
    ap.add_argument(
        "--limit", type=float, help="explicit limit price (ignored for --market)"
    )
    ap.add_argument(
        "--offset", type=float, default=0.02, help="BID - offset (ignored for --market)"
    )
    ...
    for attempt in range(1, 4):
        try:
            resp = et.place_equity_order(
                prev
            )  # qty=None; wrapper will reuse preview qty
