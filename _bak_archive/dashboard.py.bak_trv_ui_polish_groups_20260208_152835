...
def api_trade_review():
    ...
    if not buy_path and not sell_path:
        return jsonify({"ok": False, "realized": (lambda r: (r if isinstance(r, dict) else {}))(
                (lambda r: (r | {"year": r.get("year") or {"pnl": 0.0, "pct": 0.0}}) if hasattr(r, "get") else {"year": {"pnl": 0.0, "pct": 0.0}})(realized)
            ),
            
            "error": "No logs found under .\\logs (triggers_*.csv / sell_triggers_*.csv)."}), 200
...
if __name__ == "__main__":
    ...
    # IMPORTANT: disable reloader to avoid zombie/duplicate listeners on Windows
...
# (deduped duplicate api_settings_scoreboard)


# (deduped duplicate healthz)
