...
# --- file paths (use your existing JSONs) ---
BASE_DIR = os.path.dirname(__file__)
BUY_JSON  = os.path.join(BASE_DIR, 'live_settings.json')          # Buy page
SELL_JSON = os.path.join(BASE_DIR, 'sell_guard_settings.json')    # Sell page


# ---------- nested settings helpers ----------
def _load_json_silent(path):
    try:
...
def _save_json_atomic(path, data: dict):
    tmp = path + ".tmp"
    with open(tmp, 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=2, sort_keys=True)
    os.replace(tmp, path)

def _coerce_like(example_val, new_str):
    # Coerce posted string to the same type as example_val
    if isinstance(example_val, bool):
        return str(new_str).lower() in ('1', 'true', 'on', 'yes')
    if isinstance(example_val, int) and not isinstance(example_val, bool):
        try:
            return int(float(new_str))
        except:
            return example_val
    if isinstance(example_val, float):
        try:
            return float(new_str)
        except:
            return example_val
    if isinstance(example_val, (list, dict)):
        # allow JSON editing in <textarea>
        try:
...
def _walk_bool_paths(d, prefix=""):
    """Return set of dotted paths that are booleans, to handle unchecked boxes."""
    paths = set()
...
def _get_by_path(d, path):
    ...
    for part in path.split('.'):
        ...
        parent, key = cur, part
        cur = cur[part]
    return parent, key  # for in-place set

def _apply_form_to_nested_settings(current: dict, form: dict) -> dict:
    """
    Update nested dict using dotted keys like 'sell_guard.mode' or 'risk.trail_pct'.
    Types are coerced against the existing example values.
    Unchecked checkboxes become False (we detect by existing boolean paths).
    """
    updated = json.loads(json.dumps(current))  # deep copy

    # 1) Apply posted fields
    for k, v in form.items():
        if k.startswith('_'):  # ignore helper fields
            continue
        parent, key = _get_by_path(updated, k)
        if parent is not None:
            parent[key] = _coerce_like(parent[key], v)
        else:
            # create shallow path if totally new (rare)
            # we’ll store as string/number heuristic
            node = updated
            parts = k.split('.')
            for part in parts[:-1]:
            ...
            try:
                ...
                try:
                    node[parts[-1]] = float(v) if '.' in v else int(v)
                except:
                    node[parts[-1]] = v

    # 2) Force unchecked checkboxes (missing from POST) to False
    bool_paths = _walk_bool_paths(current)
    for bp in bool_paths:
        if bp not in form:
            parent, key = _get_by_path(updated, bp)
            if parent is not None:
                parent[key] = False
...
def _coerce_like_example(example_val, new_val_str):
    # Coerce a posted string to the same type as the example value
    if isinstance(example_val, bool):
        return new_val_str.lower() in ('1', 'true', 'on', 'yes')
    if isinstance(example_val, int):
        try: return int(float(new_val_str))
        except: return example_val
    if isinstance(example_val, float):
        try: return float(new_val_str)
        except: return example_val
    return new_val_str  # strings/others
...
def _apply_form_to_settings(current: dict, form: dict) -> dict:
    updated = dict(current)
    for k, v in form.items():
        if k.startswith('_'):  # ignore CSRF/hidden helpers if any
            continue
        if k in current:
            updated[k] = _coerce_like_example(current[k], v)
        else:
            # heuristics for new keys
            low = v.lower()
            if low in ('true','false','on','off','yes','no'):
                updated[k] = (low in ('true','on','yes'))
            else:
                try:
                    if '.' in v: updated[k] = float(v)
                    else: updated[k] = int(v)
                except:
                    updated[k] = v
    # Also handle unchecked checkboxes (missing from form)
    for k, val in current.items():
        if isinstance(val, bool) and k not in form:
            updated[k] = False
    return updated
...
@app.route('/settings/buy', methods=['GET', 'POST'])
def buy_settings():
    cur = _load_json_silent(BUY_JSON)
    if request.method == 'POST':
        new = _apply_form_to_nested_settings(cur, request.form.to_dict())
        _save_json_atomic(BUY_JSON, new)
        flash('Buy settings saved.', 'ok')
        return redirect(url_for('buy_settings'))
    return render_template(
        'settings_generic.html',
        title='Buy Settings',
        root_key=None,            # no forced root section
        settings=cur,
        save_url=url_for('buy_settings')
    )

@app.route('/settings/sell', methods=['GET', 'POST'])
def sell_settings():
    cur = _load_json_silent(SELL_JSON)
    if request.method == 'POST':
        new = _apply_form_to_nested_settings(cur, request.form.to_dict())
        _save_json_atomic(SELL_JSON, new)
        flash('Sell settings saved.', 'ok')
        return redirect(url_for('sell_settings'))

    # If your file has {"sell_guard": {...}}, we’ll pass that whole dict for rendering
    # but keep dotted names so form posts as "sell_guard.xxx"
    return render_template(
        'settings_generic.html',
        title='Sell Settings',
        root_key=None,
        settings=cur,             # render whole file; template will recurse
        save_url=url_for('sell_settings')
    )
...
def live_status():
    ...
    try:
        ...
        def build_account_for_ui() -> dict:
            ...
            # Pull raw numbers
            bp  = _num(ui.get("available_funds")) \
                  or _num(ui.get("marginBuyingPower")) \
                  or _num(summ.get("buying_power")) \
                  or 0.0

            pos = _num(ui.get("positions_value")) \
                  or _num(summ.get("positions_value")) \
                  or 0.0

            # Your preference:
            #   Equity Value (card) = Buying Power + Positions Value
            equity = round((bp or 0.0) + (pos or 0.0), 2)

            # Your preference for CASH accounts: always show 0.00 here
            settled_display = 0.0

            # normalize core numbers from the summary-ui
            def _f(x):
                try:
            ...
            # ---- normalize balances for UI ----
            bp = (
                _num(ui.get("cashAvailableForInvestment"))
                or _num(ui.get("available_funds"))
                or _num(ui.get("cashAvailableForWithdrawal"))
                or _num(ui.get("cashBuyingPower"))
                or _num(summ.get("buying_power"))
                or 0.0
            )

            posv = (
                _num(summ.get("positions_value"))
                or _num(ui.get("positions_value"))
                or _num(ui.get("netMv"))
                or 0.0
            )

            value_total = round(bp + posv, 2)

            return {
                "account_id": acct_id,
                "account_key": acct_key,
                "account_type": ident.get("account_type") or ui.get("account_type") or "",
                "account_type_display": ident.get("account_type_display") or ui.get("account_type_display") or "",

                # keep only what the card needs
                "value_total": value_total,        # <-- rename card to "Value"
                "buying_power": bp,                # Buying Power = cashAvailableForInvestment
                "positions_value": posv,           # move from 2nd tile into this one

                # leave raw for anything else
                "ui": ui,
            }
        def _num(x):
