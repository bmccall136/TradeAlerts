...
def _ms(x):
    try:
        if isinstance(x, (int,float)): v=float(x); return int(v if v>1e10 else v*1000)
        s=str(x or "").strip()
        if not s: return 0
        if " " in s and "T" not in s: s=s.replace(" ","T")
        if "Z" not in s and "+" not in s: s+="Z"
...
print(f"[CFG] take_profit={tp_pct:.2f}%  stop_loss={sl_pct:.2f}%  min_hold={min_hold_min}m")
...
def last_for(sym):
    try:
        ...
        qd  = (raw.get("QuoteResponse") or {}).get("QuoteData") or raw.get("QuoteData") or []
        if isinstance(qd, dict): qd = [qd]
        last = None; prev = None
        if qd:
...
for s, qty in sorted(inv.items()):
    ...
    print(f"{s:6}  {q:3d}  {last if last else '-':>6}  {basis if basis else '-':>6}  {pnl_pct:6.2f}%  {held_min:8}   {decision}  [{reason}]")
