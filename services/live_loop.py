...
def run_live_loop(settings, symbols, broker_mode=None):
    ...
    while True:
        ...
        holdingsL = {s: {"qty": q, "avg_cost": ac, "last_price": lp} for s, q, ac, lp in get_holdings()}
        ...
        for sym, price, triggered in ranked:
            ...
            bp_str = f"{live_bp:.2f}" if isinstance(live_bp, (int, float)) else "None"
            sc_str = f"{settled_cash:.2f}" if isinstance(settled_cash, (int, float)) else "None"
            mpt_str = f"{max_per_trade:.1f}" if isinstance(max_per_trade, (int, float)) and max_per_trade != float("inf") else "inf"
            log.info("[LIVE] considering %-6s px=%.2f afford_qty=%d settled=%s bp=%s max_per_trade=%s safe_cap=%s -> qty=%d",
                     sym, float(price), afford_qty, sc_str, bp_str, mpt_str, (safe_cap if safe_cap is not None else "N/A"), qty)
