...
def _from_any(bal: Dict[str, Any], *keys: str) -> Optional[float]:
    comp = _get_comp(bal)
    for k in keys:
...
def _extract_buying_power_from_snapshot(snapshot: dict) -> float | None:
    ...
    keys = (
        ...
        "cashBuyingPower", "buyingPower", "availableFundsForTrading",
        "marginBuyingPower", "bp",
...
def _dig_numbers(blob: dict, key_names) -> dict:
    ...
    def norm(k: str) -> str: return re.sub(r'[_\s]', '', str(k).lower())
...
def _fetch_account_snapshot(broker) -> dict:
    ...
    try:
        ...
        if isinstance(comp, dict) or isinstance(cash, dict):
            return {"Computed": comp, "Cash": cash, **{k: v for k, v in snap.items() if k not in ("Computed","Cash")}}
    except Exception:
        pass
    ...
    try:
        ...
        acct_id = getattr(broker, "_account_id_key", None) or getattr(broker, "account_id", None)
        if et and acct_id:
...
def _extract_settled_cash(acct: dict) -> float | None:
    ...
    """
    hits = _dig_numbers(acct, [
        "moneyMktBalance", "netCash",
        "settledCash", "settledCashForInvestment",
        "cashBalance", "cashAvailableForInvestment",
        "computedCashAvailableForInvestment",
    ])
    prefer = ("moneyMktBalance", "netCash", "settledCash", "settledCashForInvestment",
              "cashBalance", "cashAvailableForInvestment", "computedCashAvailableForInvestment")
    for want in prefer:
        for k, v in hits.items():
            if k.lower().replace("_","") == want.lower().replace("_","") and v is not None and v >= 0:
                return float(v)
    return None
...
def _extract_buying_power(acct: dict) -> float | None:
    ...
    """
    hits = _dig_numbers(acct, ["cashBuyingPower","marginBuyingPower","netCash","buyingPower"])
...
def run_live_loop(settings, symbols, broker_mode=None):
    ...
    cand_limit = _env_int(
    "LIVE_CANDIDATE_LOG_LIMIT",
    sget(settings, "candidate_log_limit", -1)
    )
    ...
    while True:
        ...
        def _affordable_qty(px: float, max_per_trade: float) -> int:
            if not isinstance(px, (int, float)) or px <= 0:
                return 0
            cap = float(max_per_trade) if isinstance(max_per_trade, (int, float)) else float("inf")
        ...
        for sym, price, triggered in ranked:
            ...
            log.info(
                ...
                sym, float(price), afford_qty, sc_str, bp_str,
