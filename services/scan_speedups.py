...
def log_candidate(sym: str, price: float, triggered: list[str], scanned_i: int, total: int) -> None:
    ...
    try:
        append_trigger_row(
            symbol=sym,
            price=price,
            triggered=pretty,  # mapper is tolerant
            source="speed_scan",
...
def _parse_etrade_quote_payload(payload: dict) -> dict[str, float]:
    ...
    for qd in qd_list:
        try:
            ...
            # Try a handful of common keys for last price
...
def fetch_intraday_prices_et(broker, symbols: Sequence[str]) -> dict[str, float]:
    ...
    for chunk in batched(list(symbols), 100):
        try:
            # LiveBroker exposes the E*TRADE session at broker._et
...
# ── warm history (DISABLED) ──────────────────────────────────────────────────


def preload_history_yahoo(symbols: Sequence[str], months: int = 6) -> None:
    """
    Historical warmup via Yahoo is disabled. This function remains for compatibility.
    """
    return
