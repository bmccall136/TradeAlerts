...
def _norm_triggers(triggered: list[str]) -> set[str]:
    ...
    for t in triggered or []:
        ...
        if "rsi" in s:
            out.add("rsi")
        if "rsislope" in s or "rsi∠" in s:
            out.add("rsi_slope")
        if "macd" in s:
            out.add("macd")
        if "hist" in s and "macd" in s:
            out.add("macd_hist")
        if "bb" in s:
            out.add("bb")
        if "bb" in s and ("break" in s or "bo" in s):
            out.add("bb_breakout")
        if "price>sma20" in s or "price>sma(20)" in s or s == "sma20":
            out.add("price_gt_sma20")
        if "vwap" in s:
            out.add("vwap")
        if "vol" in s:
            out.add("vol_spike")
        ...
        if "gap" in s:
            out.add("gap")
        if "news" in s or "headline" in s:
            out.add("news")
    return out


def append_trigger_row(
    symbol: str,
    price: float,
    triggered: list[str],
