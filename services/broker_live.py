...
def _sesh() -> OAuth1Session:
    ...
    return OAuth1Session(
        CK, client_secret=CS, resource_owner_key=AT, resource_owner_secret=AS
    )
...
def get_account_id_key(force: bool = False) -> str:
    ...
    pick = next(
        (
            a
            for a in accounts
            if str(a.get("accountType", "")).upper().startswith("BROKER")
        ),
        accounts[0],
    )
...
def preview_equity_order(
    ...
    *,
    ...
    if priceType is None:
        priceType = (
            "LIMIT" if (price is not None or limitPrice is not None) else "MARKET"
        )
...
def place_from_preview(accountIdKey: str, preview_json: dict) -> dict:
    ...
    pr = (
        preview_json.get("PreviewOrderResponse")
        or preview_json.get("previewOrderResponse")
        or {}
    )
    preview_ids = pr.get("PreviewIds") or pr.get("previewIds") or []
    ...
    body = {
        "PlaceOrderRequest": {
            "orderType": "EQ",
            "clientOrderId": (
                pr.get("clientOrderId") or pr.get("ClientOrderId") or f"ta-{pv}"
            ),
...
def market_buy(symbol: str, qty: int) -> dict:
    ...
    p = preview_equity_order(
        acct, action="BUY", symbol=symbol, qty=qty, priceType="MARKET"
    )
...
def market_sell(symbol: str, qty: int) -> dict:
    acct = get_account_id_key()
    p = preview_equity_order(
        acct, action="SELL", symbol=symbol, qty=qty, priceType="MARKET"
    )
    return place_from_preview(acct, p)
...
def place_stop_limit(
    symbol: str, qty: int, stop_price: float, limit_price: float
) -> dict:
    acct = get_account_id_key()
    p = preview_equity_order(
        acct,
