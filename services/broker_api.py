...
def buy_stock(symbol, qty):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO trades(symbol, action, qty, price, timestamp) VALUES (?,?,?, ?,datetime('now'))",
...
def sell_stock(symbol: str, qty: int, price: float, trade_time: str = None):
    ...
    if not holding or holding.get("qty", 0) < qty:
        raise RuntimeError(
            f"Not enough shares to sell for {symbol} (have {holding.get('qty', 0)}, tried to sell {qty})"
        )
