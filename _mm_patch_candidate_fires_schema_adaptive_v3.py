...
bak = P.with_suffix(f".py.bak_candidate_fires_schema_adaptive_v3_{ts}")
shutil.copy2(P, bak)
print(f"Backup -> {bak}")
...
# Find the existing START..END block and capture its indentation
...
blk = (
    ...
    f"{ind}\n"
    f"{ind}    # Introspect schema so we don't assume columns that don't exist.\n"
    ...
    f"{ind}            sig_total = None\n"
    f"{ind}\n"
    f"{ind}    # by_indicator (schema-adaptive)\n"
    f"{ind}    by_indicator = {{}}\n"
    f"{ind}    if group_col:\n"
    f"{ind}        q = (\n"
    ...
    f"{ind}\n"
    f"{ind}    # rows (only select columns that exist)\n"
    ...
    f"{ind}        f\"SELECT {sel} FROM trigger_fires \"\n"
    f"{ind}        \"WHERE CAST(ts_utc AS INTEGER) >= ? AND CAST(ts_utc AS INTEGER) < ? \"\n"
    ...
    f"{ind}    if _debug == '1':\n"
    f"{ind}        payload['debug'] = {{\n"
    f"{ind}            'db': _db, 'rows_returned': len(rows), 'since': int(_since), 'until': int(_until),\n"
    f"{ind}            'trigger_fires_cols': cols, 'group_col': group_col\n"
    f"{ind}        }}\n"
    f"{ind}    return jsonify(payload)\n"
    f"{ind}except Exception as e:\n"
    f"{ind}    # Return 200 so Invoke-WebRequest doesn't throw\n"
    f"{ind}    try:\n"
    f"{ind}        from flask import jsonify\n"
    f"{ind}        return jsonify({{\n"
    f"{ind}            'ok': False,\n"
    f"{ind}            'error': str(e),\n"
    f"{ind}            'errors': [str(e)],\n"
    f"{ind}            'rows': [],\n"
    ...
    f"{ind}            'cand': 0,\n"
    f"{ind}            'req': 0,\n"
    f"{ind}            'counts': {{'trigger_fires': 0, 'signal_fires': 0}},\n"
    f"{ind}        }})\n"
    f"{ind}    except Exception:\n"
    f"{ind}        raise\n"
    f"{ind}# {END}\n"
...
new_txt, n = pat.subn(blk, txt, count=1)
if n != 1:
    raise SystemExit(f"ERROR: Replace count={n} (expected 1).")

P.write_text(new_txt, encoding="utf-8")
print(r"PATCHED -> C:\TradeAlerts\dashboard.py")
print("Replaced V2S block -> schema-adaptive V3")
