...
def apply_fill(fill: dict, ignore: set | None = None):
    ...
    if side == "BUY":
        ...
        pl = (px * qty) - cost_used
        pl_pct = (
            ((px - tr_price_paid) / tr_price_paid * 100.0) if tr_price_paid else 0.0
        )
...
def summarize_today():
    ...
    realized = sum(
        float(t.get("pl") or 0.0)
        for t in trades
        if (t.get("action") or "").upper() == "SELL"
    )
    basis = sum(
        (float(t.get("price_paid") or 0.0) * int(t.get("qty") or 0))
        for t in trades
        if (t.get("action") or "").upper() == "SELL"
    )
