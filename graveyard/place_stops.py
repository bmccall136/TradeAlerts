...
def main():
    ...
    for p in positions:
        ...
        pr = (
            prev.get("PreviewOrderResponse")
            or prev.get("PreviewOrderResponseV2")
            or prev
        ) or {}
        pid = pr.get("previewId") or (
            (pr.get("PreviewIds") or [{}])[0].get("previewId")
        )
        if not pid:
        ...
        print(
            "  ✅ placed:",
            (placed.get("PlaceOrderResponse") or {}).get("orderId") or "OK",
        )
        armed += 1
