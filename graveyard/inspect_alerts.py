...
for row in c.execute(
    "SELECT symbol, signal, price, vwap, timestamp FROM alerts ORDER BY timestamp DESC LIMIT 10"
):
    print(row)
...
for row in c.execute(
    "SELECT symbol, signal, price, vwap, timestamp FROM alerts WHERE timestamp LIKE ? ORDER BY timestamp DESC",
    (today + "%",),
):
    print(row)
