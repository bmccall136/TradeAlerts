...
from services.alert_service import clear_alerts_by_filter, get_alerts, insert_alert
...
def sparkline(alert_id):
    ...
    ys = [
        (height - (v - lo) / (hi - lo) * height) if hi > lo else height / 2
        for v in data
    ]
    path = "M" + " L".join(f"{x:.1f},{y:.1f}" for x, y in zip(xs, ys, strict=False))
