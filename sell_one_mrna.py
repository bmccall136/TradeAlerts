...
def main():
    ...
    ap.add_argument(
        "--place", action="store_true", help="Place the order (otherwise just preview)."
    )
    args = ap.parse_args()
    ...
    if qty < 1:
        print(
            "You don't appear to have a free long share of MRNA (qty found:", qty, ")."
        )
    ...
    for fn_name in ("place_equity_order", "place_order"):
        ...
        try:
            used = {
                k: v for k, v in order_kwargs.items() if k in fn.__code__.co_varnames
            }
            placed = fn(**used)
    ...
    for name, err in errors:
        print(" ", name, "->", err)
    print(
        "If you have a different place function, please share its name/signature (dir(etrade_service))."
    )
