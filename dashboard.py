...
def simulation():
    # load the summary numbers
    cash           = get_cash()
    unrealized_pnl = get_unrealized_pl()
    realized_pnl   = get_realized_pl()
    print(f"[DEBUG] Cash in DB: {cash}")

    # pull current positions and trade history
    raw_h = get_holdings()
    # turn them into dicts your template expects
    holdings = [
        {
            'symbol'     : sym,
            'qty'        : qty,
            ...
            'change_pct' : (last_price - avg_cost) / (avg_cost or 1) * 100,
            'value'      : last_price * qty,
        }
        for sym, qty, avg_cost, last_price in raw_h
    ]
