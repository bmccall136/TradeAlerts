...
def _dedupe_trades(rows):
    seen, out = set(), []
    for t in rows or []:
        uid = str(t.get("uid") or "").strip() or "|".join(
            [
                str(t.get("time_ms") or t.get("time") or ""),
...
def get_account_summary():
    ...
    return _get(
        f"/v1/accounts/{aid}/balance.json", params={"instType": "BROKERAGE"}
    ).json()
...
def get_positions():
    aid = _primary_account_id()
    return _get(
        f"/v1/accounts/{aid}/portfolio.json", params={"instType": "BROKERAGE"}
    ).json()
...
def _normalize_positions_payload(payload) -> list[dict]:
    ...
    if isinstance(payload, list):
        ...
        if (
            ...
            and payload[0].get("symbol") is not None
        ):
            return payload
...
def live_status():
    ...
    try:
        ...
        def _sum_realized_between(
            trades: list[dict], start_d: date, end_d: date
        ) -> float:
        ...
        def _fifo_enrich_and_daytrades(trades):
            ...
            Returns: enriched_trades, realized_today_val, realized_today_pct, daytrade_stats
            """
            ...
            def live_status():
                ...
                current_app.logger.info(
                    "[LIVE] transactions_as_trades -> %d", len(trades)
                )
        ...
        bp = _first_num(
            comp.get("marginBuyingPower"), comp.get("cashBuyingPower"), b._fresh_funds()
        )
        avail_wd = _first_num(
        ...
        settled_cash = _first_num(
            comp.get("netCash"), comp.get("cashBalance"), cash.get("moneyMktBalance")
        )
                ...
                for s in missing:
                    try:
                        ...
                        if not _merge_quotes(raw, f"one:{s}"):
                            ...
                            try:
                                qd = (raw.get("QuoteResponse") or {}).get(
                                    "QuoteData"
                                ) or []
                                ...
                                if qd:
                                    ...
                                    last = _to_f(
                                        allb.get("lastTrade") or allb.get("lastPrice")
                                    )
                                    prev = _to_f(
        ...
        try:
            ...
            _log(
                "warning", "[LIVE] holdings enrichment failed: %s", e
            )  # --------------- Rollups -----------------------
        ...
        if not trades_src:
            try:
                ...
                _log(
                    "info", "[LIVE] recent_executions_as_trades -> %d", len(trades_src)
                )
            except Exception as e:
        ...
        try:
            ...
            current_app.logger.info(
                "[LIVE] transactions_as_trades -> %s", len(trades_rows)
            )
        except Exception as e:
        ...
        if not trades_rows:
            try:
                ...
                current_app.logger.info(
                    "[LIVE] recent_executions_as_trades -> %s", len(trades_rows)
                )
            except Exception as e:
        ...
        try:
            trades_enriched, realized_today, realized_today_pct = _fifo_realized_today(
                trades_rows
            )
        except Exception as e:
            current_app.logger.warning(
                "[LIVE] FIFO enrich failed, using raw trades: %s", e
            )
        ...
        trades_for_metrics = (
            ...
            else locals().get("trades") or []
        )
        metrics = (
            payload.get("metrics")
            if isinstance(payload, dict)
            else locals().get("metrics") or {}
        )
        ...
        metrics["realized_buckets"] = _realized_buckets(
            trades_for_metrics, equity_baseline
        )
