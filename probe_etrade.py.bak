...
def accounts_first(accts_json):
    ...
    try:
        ar = accts_json.get("AccountListResponse") or accts_json
        al = ar.get("Accounts") or {}
        items = al.get("Account") or []
...
def get_balance_portfolio(aid_key, aid_num):
    ...
    def try_one(aid, realtime: bool):
        ...
        which = f"{'key' if aid == aid_key else 'id'}|realtime={realtime}"
        bal = pf = None
        err_b = err_p = None
        # balance
        try:
            bal = eget_path(f"/v1/accounts/{aid}/balance.json", params)
        except requests.HTTPError as e:
            try:
                err_b = e.response.json()
            except Exception:
                err_b = {"raw": str(e)}
        # portfolio
        try:
    ...
    results = []
    for aid in [a for a in (aid_key, aid_num) if a]:
        for realtime in (True, False):
            results.append(try_one(aid, realtime))
...
def main():
    # 1) Accounts list
    acc_list = eget_path("/v1/accounts/list.json") or {}
    dump("accounts.list", acc_list)
    aid_key = os.getenv("ETRADE_ACCOUNT_ID_KEY") or aid_key
    aid_num = os.getenv("ETRADE_ACCOUNT_ID") or aid_num
    print(f"Using accountIdKey: {aid_key}\nAlso have numeric accountId: {aid_num}")
    ...
    # allow CLI override
    aid_key = acct.get("accountIdKey")
    aid_num = acct.get("accountId")
    if len(sys.argv) > 1:
        # user may pass either form; if it’s numeric-ish, treat as id_num
        arg = sys.argv[1]
        if arg.isdigit():
            aid_num = arg
    ...
    print(f"\nUsing accountIdKey: {aid_key}")
    if aid_num:
        print(f"Also have numeric accountId: {aid_num}")

    # 2) Balance + Portfolio with fallback
    bal, pf = get_balance_portfolio(aid_key, aid_num)
    dump("accounts.balance", bal)
    dump("accounts.portfolio", pf)

    # 3) Quotes (multi + single)
    symbols = ["COP", "KO", "INTC", "FCX", "DAY", "MRNA"]
    q_multi = quotes_multi(symbols); dump("market.quote.multi", q_multi)
    time.sleep(1)
    q_one   = quote_single("COP");   dump("market.quote.COP", q_one)
