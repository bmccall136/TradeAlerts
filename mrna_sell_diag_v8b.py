...
def place_with_retries(fn, payload, debug=False, tries=3, delay=0.8):
    for i in range(1, tries+1):
        try:
            ...
            msg = str(e)
            if debug: print(f"[DBG] place(preview_full) attempt {i} failed:", msg)
...
def main():
    ...
    ap.add_argument("--place-mode", choices=["full","inner","auto"], default="full", help="placement shape to use")
    ...
    if args.place_mode == "full":
        ...
        inner = prev.get("PreviewOrderResponse") or prev.get("previewOrderResponse")
        if not isinstance(inner, dict):
            print("[SELL] no PreviewOrderResponse in preview"); return
        # attempt inner with light retries
        ok = None; how = ""
        for i in range(1,3):
            try:
                ok = fn(inner); how = f"place(preview_inner) attempt {i}"; break
            except Exception as e:
                if args.debug: print(f"[DBG] place(preview_inner) attempt {i} failed:", e)
                time.sleep(0.6)
        resp = ok
        if resp is None:
            resp, how = place_fallbacks(acct_key, prev, debug=args.debug)
